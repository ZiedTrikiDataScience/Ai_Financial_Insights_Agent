...
if st.button("\U0001F680 Run Full Pipeline", type='primary', use_container_width=True):
    ...
    st.subheader("Step 3: Embed & Cluster Summaries")
    ...
    df["theme_id"] = KMeans(n_clusters=num_themes, random_state=42).fit_predict(embeddings)
    st.success(f"Generated {num_themes} clusters")

    import re  # add at the top of the file if not already
