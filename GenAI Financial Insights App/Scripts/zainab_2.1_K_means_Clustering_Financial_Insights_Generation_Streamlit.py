...
# ==== Build Ground-Truth Excel (bodyText vs trailText) ====
st.subheader("Build Ground Truth (for summarization eval)")
st.caption("Colonne 1 = article (bodyText), Colonne 2 = expected output (trailText).")

if st.button("Create ground_truth.xlsx", use_container_width=True):
    # 1) Scrape depuis The Guardian avec trailText
    ...
    params = {
        ...
        "page-size": 200,             # ajuste si tu veux plus/moins d'articles
    ...
    # 2) Construire le dataframe : article vs expected output
    rows = [
        {
    ...
    # 3) Sauvegarde Excel + bouton de téléchargement
    out_path = Path(tempfile.gettempdir()) / "ground_truth.xlsx"
    gt_df.to_excel(out_path, index=False)

    st.success(f"Ground truth créé ({len(gt_df)} lignes).")
    st.dataframe(gt_df.head(5))

    with open(out_path, "rb") as f:
        st.download_button(
            "⬇️ Download ground_truth.xlsx",
            f,
